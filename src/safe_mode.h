 ...
 * shell answers by bringing up nothing else: the USB port is up before
 * setup() runs, so a firmware that dies before the loop can still be
 * reflashed over the cable. The count clears once a boot has run for
 * SAFE_MODE_SETTLED_MS, and safe mode clears it too, so the boot after safe
 * mode is a normal one.
 */
