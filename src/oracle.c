...
const die_t DICE[] = {
    {"D2", DIE_NUMERIC, 2},     {"D4", DIE_NUMERIC, 4},
    {"D6", DIE_NUMERIC, 6},     {"D8", DIE_NUMERIC, 8},
    {"D10", DIE_NUMERIC, 10},   {"D12", DIE_NUMERIC, 12},
    {"D20", DIE_NUMERIC, 20},   {"D66", DIE_D66, 0},
    {"D100", DIE_NUMERIC, 100}, {"ORACLE", DIE_ORACLE, 0},
};
...
static const struct {
    ...
    {"NO", "and"}, {"NO", NULL},  {"NO", "but"},
    {"YES", "but"}, {"YES", NULL}, {"YES", "and"},
};
...
void oracle_begin(void) { bootloader_random_enable(); }

// One 32-bit draw: hardware RNG mixed with the CPU cycle counter, which at
// roll time carries the instant of the tap.
static uint32_t draw(void) { return esp_random() ^ esp_cpu_get_cycle_count(); }
...
roll_t roll_die(const die_t *die) {
    roll_t roll = {.kind = die->kind, .answer = "", .modifier = NULL};

    switch (die->kind) {
        case DIE_ORACLE: {
            const uint32_t index = roll_below(6);
