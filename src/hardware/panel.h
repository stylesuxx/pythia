 ...
 *
 * The panel's output follows it: enabled before the first lit level and
 * disabled once the level reaches 0, so the panel stops driving the glass.
 * Display memory survives either way and still accepts frames, so whatever
 * was last presented is on the glass the moment the light comes back. An
 * unchanged level costs nothing.
 */
